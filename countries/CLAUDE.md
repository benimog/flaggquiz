...
│   ├── QuizOptions.tsx        # Collapsible settings disclosure (summary line + scope/mode/difficulty/direction toggles) for the four multiple-choice quizzes
│   ├── ChoiceGrid.tsx         # 2×2 answer-button grid for the text quizzes; long names wrap instead of overflowing on mobile
...
- **Flag quiz game modes**: The four multiple-choice quizzes (`FlagQuiz`, `ReverseFlagQuiz`, `Continents`, `CapitalQuiz`) share `useFlagQuizGame`. Standard mode (default) plays the shuffled pool through once — each country appears exactly once — then shows `GameOverDialog`. Practice mode (`t("quiz.practiceMode")`, "Testläge") is the never-ending variant: it reshuffles and continues forever. Each component owns a `practice` boolean state and a `standard`/`practice` toggle. The array passed to the hook must be referentially stable (module-level getter, module-level filter, or `useMemo`) — an inline `.filter()`/`.map()` would restart the deck every render and break standard mode. All four also own a `hard` boolean (`normal`/`hard` toggle); hard mode passes `getSubregion` as the hook's `getGroupKey` (and `undefined` otherwise — both referentially stable, so toggling cleanly restarts the deck). The settings toggles are rendered through the shared **`QuizOptions`** disclosure, not raw stacked `ToggleButtonGroup`s. Collapsed by default it shows a one-line summary of the current selections (the selected option labels joined with `·`) plus an "Options" (`t("quiz.optionsLabel")`, "Alternativ") toggle; expanded it reveals captioned segmented controls (scope/direction/mode/difficulty) that stack on mobile and form a row on desktop. Pass each control as `{ caption, value, options, onChange }`; non-setting actions (e.g. Continents' "change continent" navigation) stay outside it. The toggles stay mounted while collapsed (MUI `Collapse`, no `unmountOnExit`), and their button text is unchanged from before, so the `controlLabels` set in the affected `*.test.tsx` still isolates the choice buttons — **but the "Alternativ" trigger is also a button, so it must be in `controlLabels`** (only Continents' test uses that finder). To interact with a toggle in a test, click "Alternativ" first to expand. Keep these in sync when adding or renaming a control; captions are `Typography`, not buttons. The four text-answer choices render through the shared **`ChoiceGrid`** (FlagQuiz, CapitalQuiz, Continents) — a `repeat(2, minmax(0, 1fr))` CSS grid with `gridAutoFlow: column`, so long country names wrap inside the button instead of overflowing the viewport on mobile, while the column-major visual order (choices 0/1 left, 2/3 right) keeps the numpad shortcuts (7=0, 4=1, 8=2, 5=3) aligned. ReverseFlagQuiz keeps its own fixed-size flag-image buttons.
